...
md5::md5(const void *input, std::size_t length)
{
	reset();
	update(input, length);
}
...
context. 
*/  
void md5::update(const uint8_t *input, std::size_t length) 
{  
...
a multiple of 4. 
*/  
void md5::encode(const uint32_t *input, uint8_t *output, std::size_t length) 
{  
...
a multiple of 4. 
*/  
void md5::decode(const uint8_t *input, uint32_t *output, std::size_t length) {  
...
std::string md5::toString(bool bUpper /*= false*/) {  
	return gtc::convert::toStringHexA((const char *)digest(), 16, bUpper);
