...
static uint ReadCharFromStream( StreamIn* in )
{
    ...
    */
    ...
    if ( in->encoding == UTF8 )
    {
        ...
        if (!err && (n == (uint)EndOfStreamTidy) && (count == 1)) /* EOF */
            return EndOfStreamTidy;
        else if (err)
        {
            /* set error position just before offending character */
