...
{
    while ( config->c != EndOfStreamTidy )
    {
        config->c = GetC( config );
        if ( config->c == '\n' || config->c == '\r' )
            break;
...
static uint NextProperty( TidyConfigImpl* config )
{
    ...
    {
        /* skip to end of line */
        while ( config->c != '\n' &&  config->c != '\r' &&  config->c != EndOfStreamTidy )
             config->c = GetC( config );
...
int ParseConfigFileEnc( TidyDocImpl* doc, ctmbstr file, ctmbstr charenc )
{
    ...
    else
    {
        ...
        for ( c = SkipWhite(cfg); c != EndOfStreamTidy; c = NextProperty(cfg) )
        {
            uint ix = 0;
            ...
            if ( c == ':' )
            {
                ...
                else
                {
                    if (NULL != doc->pOptCallback)
                    {
                        ...
                        while ( i < sizeof(buf)-2 && c != EndOfStreamTidy && c != '\r' && c != '\n' )
                        {
                            if ( delim && c == delim )
                                break;
...
Bool ParseNewline( TidyDocImpl* doc, const TidyOptionImpl* entry )
{
    ...
    while ( c!=EndOfStreamTidy && cp < end && !IsWhite(c) && c != '\r' && c != '\n' )
    {
        *cp++ = (tmbchar) c;
        c = AdvanceChar( cfg );
    }
...
Bool ParseName( TidyDocImpl* doc, const TidyOptionImpl* option )
{
    ...
    while ( i < sizeof(buf)-2 && c != EndOfStreamTidy && !IsWhite(c) )
    {
        buf[i++] = (tmbchar) c;
...
Bool ParseTagNames( TidyDocImpl* doc, const TidyOptionImpl* option )
{
    ...
    do
    {
        ...
        */
        ...
        while ( i < sizeof(buf)-2 && c != EndOfStreamTidy && !IsWhite(c) && c != ',' )
        {
            buf[i++] = (tmbchar) c;
            c = AdvanceChar( cfg );
        }
...
Bool ParseDocType( TidyDocImpl* doc, const TidyOptionImpl* option )
{
    ...
    /* read first word */
    while ( i < sizeof(buf)-1 && c != EndOfStreamTidy && !IsWhite(c) )
    {
        buf[i++] = (tmbchar) c;
        c = AdvanceChar( cfg );
    }
