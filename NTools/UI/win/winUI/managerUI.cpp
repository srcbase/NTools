...
std::wstring paintManagerUI::m_pStrResourcePath;
std::wstring paintManagerUI::m_pStrResourceZip;
...
std::wstring paintManagerUI::getInstancePath()
{
...
std::wstring paintManagerUI::getCurrentPath()
{
	TCHAR tszModule[MAX_PATH + 1] = { 0 };
...
const std::wstring& paintManagerUI::getResourcePath()
{
	return m_pStrResourcePath;
}

const std::wstring& paintManagerUI::getResourceZip()
{
...
bool paintManagerUI::addOptionGroup(PCWSTR pStrGroupName, controlUI* pControl)
{
	...
	std::map<std::wstring, std::vector<controlUI *>*>::iterator itfind = m_mOptionGroup.find(pStrGroupName);
...
void paintManagerUI::removeAllOptionGroups()
{
	...
	//	}
	//}
	std::map<std::wstring, std::vector<controlUI *>*>::iterator iter ;
...
const TImageInfo* paintManagerUI::getImage(PCWSTR bitmap)
{
	...
	//else return data;
	std::map<std::wstring, TImageInfo *>::iterator itfind = m_mImageHash.find(bitmap);// std::find(m_mImageHash.begin(), m_mImageHash.end(), bitmap);
...
const TImageInfo* paintManagerUI::addImage(PCWSTR bitmap, PCWSTR type, DWORD mask)
{
	...
	//}
	std::map<std::wstring, TImageInfo *>::iterator itfind = m_mImageHash.find(bitmap);//std::find(m_mImageHash.begin(), m_mImageHash.end(), bitmap);
	//m_mImageHash.insert(itfind, std::pair<std::wstring, TImageInfo *>(, data));
...
void paintManagerUI::removeAllImages()
{
	...
	//	}
	//}
	for (std::map<std::wstring, TImageInfo *>::iterator iter = m_mImageHash.begin(); iter != m_mImageHash.end(); ++iter)
	{
...
const std::map<std::wstring, std::wstring *>& paintManagerUI::getDefaultAttribultes() const
{
...
controlUI* paintManagerUI::findControl(PCWSTR pstrName) const
{
	...
	std::map<std::wstring, controlUI *>::const_iterator itfind = m_mNameHash.find(pstrName);// std::find(m_mNameHash.begin(), m_mNameHash.end(), pstrName);
