...
void kpdeskNotify::notify(TNotifyUI& msg)
{
...
LRESULT kpdeskNotify::onSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (wParam == SC_CLOSE){
		bHandled = TRUE;
		return 0;
	...
	case WM_CREATE:
		{
			...
			controlUI* pRoot = builder.create(L"kpsk\\notify.xml", (UINT)0, NULL, &m_paintManager);   // duilib.xml需要放到exe目录下
