...
stringex richEditUI::getText() const
{
...
stringex richEditUI::getSelText() const
{
	if( !m_pTwh ) return stringex();
	CHARRANGE cr;
	cr.cpMin = cr.cpMax = 0;
	txSendMessage(EM_EXGETSEL, 0, (LPARAM)&cr, 0);
	LPWSTR lpText = NULL;
...
stringex richEditUI::getTextRange(long nStartChar, long nEndChar) const
{
	TEXTRANGEW tr = { 0 };
...
stringex richEditUI::getLine(int nIndex, int nMaxLength) const
{
	LPWSTR lpText = NULL;
