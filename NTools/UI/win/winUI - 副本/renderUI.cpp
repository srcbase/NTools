...
bool drawImage(HDC hDC, paintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const stringex& sImageName, \
	const stringex& sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, \
	...
	PCWSTR pStrImage, PCWSTR pStrModify)
{
	...
	stringex sImageName = pStrImage;
	stringex sImageResType;
	RECT rcItem = rc;
	RECT rcBmpPart = {0};
	RECT rcCorner = {0};
	DWORD dwMask = 0;
...
void renderEngine::drawHtmlText(HDC hDC, paintManagerUI* pManager, RECT& rc, PCWSTR pstrText, DWORD dwTextColor, RECT* prcLinks, stringex* sLinks, int& nLinkRects, UINT uStyle)
{
		...
		if( pt.x >= rc.right || *pstrText == L'\n' || bLineEnd ) {
			...
			if( !bLineDraw ) {
				if( bInLink && iLinkIndex < nLinkRects ) {
					...
					stringex *pStr1 = (stringex*)(sLinks + iLinkIndex - 1);
					stringex *pStr2 = (stringex*)(sLinks + iLinkIndex);
					*pStr2 = *pStr1;
				}
				...
				case L'a':  // Link
					{
						...
						if( iLinkIndex < nLinkRects && !bLineDraw ) {
							stringex *pStr = (stringex*)(sLinks + iLinkIndex);
							pStr->clear();
