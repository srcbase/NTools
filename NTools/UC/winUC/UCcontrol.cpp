...
bool controlUC::drawImage(HDC hDC, const BSImageInfo &imgInfo)
{
	Gdiplus::Graphics grap(hDC);
	gtc::image *pImage = nullptr;
	if (!imgInfo.wsKeyImage.empty() && (pImage = m_pManager->findImage(imgInfo.wsKeyImage)))
	{
		Gdiplus::Graphics grap(hDC);
		gtc::drawer::drawImage(grap, *pImage, m_rcItem, m_bsBackImage.rcScource);
		return grap.GetLastStatus() == Gdiplus::Ok;
	}
	return false;
}
