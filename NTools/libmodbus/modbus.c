...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/*****************************************************************************************
	<< --- modbus::modbus_send_straight	2014-02-08 --- >>
	说明：直接发送消息。不对消息体进行任何处理。
	参数：
	ctx			= 通讯结构
	msg			= 被发送的消息
	msg_length	= 消息长度
	返回值：状态码
*****************************************************************************************/
int modbus_send_straight(modbus_t *ctx, uint8_t *msg, int msg_length)
{
	int rc;
    do {
        rc = ctx->backend->send(ctx, msg, msg_length);
...
/*****************************************************************************************
	<< --- modbus::modbus_recv_straight	2014-02-08 --- >>
	说明：直接接收消息。不对消息体进行任何处理。
	参数：
	ctx			= 通讯结构
	msg			= 被接收的消息
	msg_length	= 被接收消息的长度
	返回值：状态码
*****************************************************************************************/
int modbus_recv_straight(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
	int rc=0;
	fd_set rfds;
    struct timeval tv;
	...
	rc = ctx->backend->select(ctx, &rfds, p_tv, rsp_length);
    if (rc == -1) {
        _error_print(ctx, "select");
        if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) {
            int saved_errno = errno;
    ...
    rc = ctx->backend->recv(ctx, rsp, rsp_length);
    if (rc == 0) {
        errno = ECONNRESET;
        rc = -1;
    }
